<?php
/**
 * Plugin Name: Cleanoded Maintenance Mode
 ...
 * Version: 1.0
 * License: GPLv2
 * Text Domain: cleancoded-maintenance-mode
 ...
 */
function cleancoded_current_user_can_bypass_vip_maintenance_mode() {
	/**
...
function vip_maintenance_mode_template_redirect() {
	if ( cleancoded_current_user_can_bypass_vip_maintenance_mode() ) {
		return;
	}
 ...
 * @since 1.0
 */
function cleancoded_vip_maintenance_mode_admin_bar_menu() {
	global $wp_admin_bar;
	...
	$wp_admin_bar->add_menu( array(
		...
		'title'  => apply_filters( 'cleancoded_vip_maintenance_mode_admin_bar_title', __( 'Under maintenance', 'maintenance-mode' ) ),
		'meta'   => array( 'class' => 'mm-notice' ),
	) );
}
add_action( 'admin_bar_menu', 'cleancoded_vip_maintenance_mode_admin_bar_menu', 8 );
 ...
 * @since 1.0
 */
function cleancoded_vip_maintenance_mode_admin_scripts() {
...
add_action( 'wp_enqueue_scripts', 'cleancoded_vip_maintenance_mode_admin_scripts' );
add_action( 'admin_enqueue_scripts', 'cleancoded_vip_maintenance_mode_admin_scripts' );
 ...
 * @return void
 */
function cleancoded_vip_maintenance_mode_load_plugin_textdomain() {
	load_plugin_textdomain( 'maintenance-mode', FALSE, basename( dirname( __FILE__ ) ) . '/languages/' );
}
add_action( 'init', 'cleancoded_vip_maintenance_mode_load_plugin_textdomain' );
